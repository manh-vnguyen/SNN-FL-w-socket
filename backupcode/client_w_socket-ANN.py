...
class PeripheralFL():
    ...
    def fit(
        ...
        while True:
            try:
                # Load data
                trainloader, testloader = cifar10_ANN.load_client_data(ARGS.node_id)

                # Set model parameters, train model, return updated model parameters
                model = cifar10_ANN.load_model().to(DEVICE)
                optimizer = torch.optim.Adam(model.parameters(), lr=0.001)

                set_parameters(model, parameters)

                cifar10_ANN.train(model, optimizer, trainloader, DEVICE, 1)
                loss, accuracy = cifar10_ANN.test(model, testloader, DEVICE)
