...
class CentralizeFL():
    ...
    def evaluate(self, global_model_params):
        test_loader = cifar10.load_test_data()
        global_model = cifar10.load_model().to(DEVICE)

        self.set_parameters(global_model, global_model_params)
    ...
    def centralized_aggregation(self, client_model_record):
        client_results = [client_model_record[uid] for uid in client_model_record.keys()]
        agged_model_params = parameters_to_ndarrays(ndarrays_to_parameters(self.fedavg_aggregate(client_results)))
        global_loss, global_acc = self.evaluate(agged_model_params)
    ...
    def start_aggregation_if_suffient_result(self):
        ...
        if len(self.client_model_record.keys()) >= self.min_fit_clients:
            ...
            self.global_aggregation_process = multiprocessing.Process(target=self.centralized_aggregation, args=(self.client_model_record,))
